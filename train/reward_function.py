...
The BehR reward measures behavioral indistinguishability by comparing the
agent's action distribution under WM-predicted states vs real states.
...
class HTTPReferenceAgent:
    ...
    def __init__(self, config: PivotGRPOConfig):
        ...
        urls = [u.strip().rstrip('/') for u in config.reference_agent_api_url.split(',')]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if models:
                    ...
                    self._model_name = model_info.get("id", self.config.reference_agent_model_path)
    ...
    def compute_behavior_consistency_rewards_batch(
        ...
        # 合并 pred + unique_real 发送给 Reference Agent
...
def compute_score(
    ...
    **kwargs
    ...
    """
    Pivot-GRPO: 综合奖励函数 (Behavior Consistency + Physical Facts + Length Penalty)
    ...
    result = {
        ...
        "page_match": 0.0,
        "rating_match": 0.0,
        "fallback_similarity": 0.0,  # 回退相似度（Reference Agent 失败时使用）
        "used_fallback": False,  # 是否使用了回退逻辑（True 表示 Reference Agent 失败）
        # === 长度监控指标 ===
    ...
    if use_full_reference_agent and ref_agent is not None and expert_action and ground_truth_str:
        ...
        result["behavior_reward"] = behavior_score
        result["fallback_similarity"] = similarity  # 记录原始相似度用于诊断
        result["used_fallback"] = True  # 标记使用了回退逻辑（无 Reference Agent 或无 expert_action）
    ...
    **kwargs
    ...
    for item in preprocessed:
        ...
        if use_full_reference_agent and ref_agent is not None and expert_action and ground_truth_str:
            ...
            # 使用回退逻辑（无 Reference Agent 或无 expert_action）
            similarity = _compute_similarity_score(solution_str, ground_truth_str)
            behavior_score = _similarity_to_behavior_reward(similarity, reward_mode)
            result["behavior_reward"] = behavior_score
            result["fallback_similarity"] = similarity
